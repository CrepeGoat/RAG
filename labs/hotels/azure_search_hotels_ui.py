...
def perform_search(
    search_client, search_text, search_mode="keyword", filters=None, top=10
):
    """
    ...
    try:
        ...
        if "semantic" in str(e).lower():
            st.info(
                "💡 If you see a semantic configuration error, your index may not have semantic search enabled. Try 'Keyword' mode instead."
            )
        else:
            st.info(
                f"💡 Search parameters used: Query='{search_text}', Mode={search_mode}, Filters={filters}"
            )
        return None
...
def display_hotel_card(hotel, show_score=False, search_mode="keyword"):
    ...
    with st.container():
        ...
        with col1:
            ...
            if search_mode in ["semantic", "semantic_filter"] and hasattr(
                hotel, "@search.reranker_score"
            ):
...
def main():
    # Page configuration
    st.set_page_config(
        page_title="Azure AI Search - Hotels Demo", page_icon="🏨", layout="wide"
    )
    ...
    if search_mode == "keyword":
        st.info(
            "💡 **Keyword Search**: Traditional BM25 ranking based on term frequency and document relevance."
        )
    elif search_mode == "semantic":
        st.info(
            "🧠 **Semantic Search**: Uses AI to understand query intent and meaning. Provides captions and reranker scores. Filters are disabled in pure semantic mode."
        )
    else:
        st.info(
            "🎯 **Semantic + Filters**: Combines semantic understanding with traditional filters for precise results."
        )
    ...
    if not filters_enabled:
        st.sidebar.warning(
            "⚠️ Filters are disabled in pure Semantic Search mode. Use 'Semantic + Filters' to enable filters."
        )
    ...
    with st.sidebar.expander("🔧 Advanced Options"):
        debug_mode = st.checkbox(
            "Show debug info", help="Display search parameters and field information"
        )
        ...
        if debug_mode:
            ...
            **Search behavior:**
            ...
            """)
            st.caption(
                "💡 Azure Search automatically uses all searchable fields defined in your index schema."
            )
    ...
    with col_auto:
        auto_search = st.checkbox(
            "🔄 Auto-search",
            value=True,
            help="Automatically search when filters or mode changes",
        )
    ...
    if should_search:
        ...
        with st.spinner(
            f"Searching hotels using {search_mode.replace('_', ' ').title()} mode..."
        ):
            ...
            if results:
                ...
                st.success(
                    f"{mode_emoji.get(search_mode, '🔍')} Found **{total_count}** hotels using **{search_mode.replace('_', ' ').title()}** mode"
                )
                ...
                if hotels:
                    ...
                    for idx, hotel in enumerate(hotels, 1):
                        ...
                        display_hotel_card(
                            hotel, show_score=show_scores, search_mode=search_mode
                        )
                else:
                    ...
                    st.info(
                        "🔍 **Search Tip**: The search looks in all fields marked as 'Searchable' in your Azure Search index. Common searchable fields include Description, Tags, and Category. If you're not finding expected results, the term may not exist in your data, or the relevant fields may not be marked as searchable."
                    )
    else:
        # Show welcome message
        if auto_search:
            st.info(
                "👆 Enter a search query or apply filters - results will update automatically"
            )
        else:
