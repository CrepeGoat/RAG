...
def test_search():
    ...
    print("-" * 60)
    try:
        results = search_client.search(search_text="*", include_total_count=True, top=5)
    ...
    print("-" * 60)
    print(
        "ℹ️  Not specifying search_fields - Azure Search will use all 'Searchable' fields"
    )
    try:
        results = search_client.search(
            search_text="pool",
    ...
    print(
        "   - Check if Description/Tags fields are marked as 'Searchable' in Azure Portal"
    )
